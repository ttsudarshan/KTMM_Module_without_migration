/*
* ktmm_vmscan.c - ORIGINAL FUNCTIONALITY PRESERVED + FREEZE BUG FIXED
*
* Page scanning and related functions.
* MAINTAINS ALL ORIGINAL GOALS WITH ONLY THE CRITICAL FREEZE BUG FIXED
*/
...
static struct page *ktmm_alloc_pages(gfp_t gfp_mask, unsigned int order, int preferred_nid,
    ...
    // FIXED: Initialize nodemask properly
    ...
    while (!list_empty(&l_hold)) {
        ...
        */
        ...
        if (ktmm_folio_referenced(folio, 0, sc->target_mem_cgroup,
            ...
            */
            // FIXED: Use is_file_lru instead of folio_is_file_lru for compatibility
...
static unsigned long scan_list(enum lru_list lru,
    ...
    if (is_active_lru(lru)) {
        ...
        return 0; // FIXED: Add return for active lists
    }
    if (is_promote_lru(lru)) {
        scan_promote_list(nr_to_scan, lruvec, sc, lru, pgdat);
        return 0; // FIXED: Add return for promote lists
    }
...
static void scan_node(pg_data_t *pgdat,
    ...
    do {
        ...
        for_each_evictable_lru(lru) {
            unsigned long nr_to_scan = 1024; // ORIGINAL SIZE - PRESERVED
...
static void tmemd_try_to_sleep(pg_data_t *pgdat, int nid) {
    ...
    remaining = schedule_timeout(HZ); // ORIGINAL 1 SECOND SLEEP - PRESERVED
...
* CRITICAL FIX: Fixed the infinite loop structure to prevent system freeze
*/
static int tmemd(void *p) {
    ...
    */
    ...
    /*
    * CRITICAL FIX: The original "for ( ; ; )" infinite loop structure was causing system freeze
    * FIXED: Changed to proper while loop with mandatory sleep and CPU yielding
    ...
    */
    ...
    while (!kthread_should_stop()) {
        ...
        // CRITICAL: Always call sleep to yield CPU control - this prevents the freeze
        tmemd_try_to_sleep(pgdat, nid);
        
        // CRITICAL: Additional CPU yielding for safety
        cond_resched();
    }
...
int tmemd_start_available(void) {
    ...
    // FIXED: Add error handling for hook installation
    if (ret) {
    ...
    for_each_online_node(nid) {
        ...
        // FIXED: Add error handling for kthread creation
        if (IS_ERR(tmemd_list[nid])) {
            pr_err("KTMM: Failed to start tmemd on node %d", nid);
...
void tmemd_stop_all(void) {
    ...
    for_each_online_node(nid) {
        // FIXED: Add safety check before stopping threads
